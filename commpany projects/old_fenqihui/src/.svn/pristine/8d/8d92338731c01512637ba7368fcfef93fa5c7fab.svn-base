controllerFunc('mainPage', 'mainController', function ($scope, $http, $timeout) {
    ...
    function getLabelInfo() {
        ...
        $http({
            ...
            url: BaseUrl + 'fqhpro/getAllProductSuperType',
        }).then(function (response) {
    ...
    function getProductList() {
        ...
        $http({
            method: 'POST',
            url: BaseUrl + 'fqhpro/getHotSellProduct',
            params:{
                start:1,
            }
        }).then(function (response) {
    ...
    $scope.searchBourn = function (lbId, name, index) {
    ...
    $scope.searchDetail = function (item) {
        go_page('../travel/travel_myDetails.html', [{'pId': item.pId}]);
    };
