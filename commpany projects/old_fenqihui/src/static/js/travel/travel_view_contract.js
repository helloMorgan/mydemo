...
controllerFunc('ngScope', 'ngController', function ($scope, $http, $timeout, $q) {
    ...
    $scope.sType = GetQueryString('pType');
    ...
    $scope.isShowRemoveTravel = true;

    if (processStatus == '303') {
        $scope.isShowRemoveTravel = false;
    }
...
//                 this.mergeArr();
        ...
        mergeArr() {
            ...
            //增加状态下
            if (processStatus == '303') {
                $scope.allImgsArr = $scope.travelImgs1;
            }
            if (processStatus == '304') {
                $scope.allImgsArr = [...$scope.receiptImgs1, ...$scope.otherImgs1];
            }
    ...
    $scope.addTravelImg2 = (imgArr, type) => {
        if (processStatus == '304') {
    ...
    $scope.removeImg2 = (imgArr, $index, type) => {
        if (processStatus == '304') {
