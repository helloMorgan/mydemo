const personalOrder = 1, toTravel = 3;
        ...
        function searchCredit() {
            $http({
                ...
                if (response.data.result == '0000') {
                    ...
                    if ($scope.startInfo < 3){
                        $scope.text_info = "您需要补充个人信用资料";
                    } else if ($scope.startInfo == 3){
                        $scope.text_info = "您的信用状况良好";
                    } else if ($scope.startInfo == 4){
                        $scope.text_info = "您的信用状况很好";
                    } else if ($scope.startInfo == 5){
                        $scope.text_info = "您的信用状况非常好";
                    }
        ...
        function getPersonalOrder(type) {
            $http({
                ...
                if (response.data.result === SUCCESS_CODE) {
                    ...
                    // }
                } else if(response.data.result == '4004') {
                    var url = "./user_login.html";
                    prompt2("登陆失效请先登陆", url);
                }
            }, (response) => {
...
//    查看详情
        $scope.checkInfo = function (item) {
            ...
            if (item.order.processStatus == 103 || item.order.processStatus == 201 || item.order.processStatus == 106) {
                ...
                go_page('../../pages/travel/travel_order_sucess_details.html', [{"oId": item.order.oId},
                    {"processStatus": item.order.processStatus},
                    {"isQianyue": "0"}
                ]);
            }
        };
