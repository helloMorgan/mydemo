...
controllerFunc('orderDetailsApp', 'orderDetailsCtrl', function ($scope, $http) {
    ...
    if (getToken()) {
        ...
        // }
        //    订单风控审核成功之前的状态
        var statusContract = function statusContract(code) {
            switch (code) {
                case '101':
                    return true;
                    break;
        ...
        $scope.goDeal = function () {
            go_page('./travel_installment_contract_text.html', [{ 'pId': $scope.productItem.product.pId }, { 'oId': $scope.productItem.order.oId }, { 'flag': 1 }, { 'isQianyue': isQianyue }, { 'processStatus': processStatus }]);
        };
        //查看产品详情
        ...
        $scope.goContract = function () {
            if (appType == 'android') {
                ...
                if (processStatus == '108') {
                    go_page('./travel_upload_contract2.html', [{ 'conId': $scope.productItem.order.conId }, { 'isQianyue': isQianyue }, { 'processStatus': processStatus }, { 'oId': $scope.productItem.order.oId }]);
                } else {
                    go_page('./travel_view_contract.html', [{ 'conId': $scope.productItem.order.conId }, { 'isQianyue': isQianyue }, { 'processStatus': processStatus }, { 'oId': $scope.productItem.order.oId }]);
                }
            }
        };
